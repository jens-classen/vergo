...
:- export(parse_pddl_domain_string/2).
:- export(parse_pddl_problem_string/2).

:- export(parse_pddl_domain_file/2).
:- export(parse_pddl_problem_file/2).

:- export(predicate_type_restriction/2).
...
parse_pddl_domain_file(InFile,OutFile) :-
        ...
        read_string(Stream, end_of_file, _Length, String),
	close(Stream),
	parse_pddl_domain_string(String,axioms(Domain_Axioms,Type_Axioms,Constant_Axioms,Predicate_Axioms,
...
parse_pddl_problem_file(InFile,OutFile) :-
        ...
        read_string(Stream, end_of_file, _Length, String),
	close(Stream),
	parse_pddl_problem_string(String,axioms(ProblemNameAxiom,DomainNameAxiom,ObjectAxioms,InitAxioms,GoalAxioms)),
        open(OutFile, write, Stream2),
...
write_time(Stream ):- write(Stream, "Generated by PDDL parser at "),
        get_flag(unix_time, T), local_time_string(T, "%c", Date),
        write(Stream, Date).
...
must_support(R) :- not(requirement(R)), !, 
	           write(error, "Requirement "), write(error, R),
		   write(error, " is necessary for this domain, but not declared!\n"),% fail.
		   write(error, "Proceeding nonetheless...\n"), flush(error).

cannot_compile(F) :- write(error, "Cannot compile feature "), write(error, F), write(error,"!\n"),
	             write(error, "Proceeding nonetheless...\n"), flush(error).
...
pddl_name_atom(Name) --> pddl_name2(CList), {string_list(String,CList), atom_string(Name,String)}.
...
ascii(String) --> {string_list(String, List)}, checkList(List).
...
remember_predicate_type_restrictions(Axioms) :-
        retract_all(predicate_type_restriction(_,_)),
...
remove_quotations(X1,X2) :- 
        term_string(X1,S1), string_list(S1,L1),
        subtract(L1,[34],L2), string_list(S2,L2),
        term_string(X2,S2).
